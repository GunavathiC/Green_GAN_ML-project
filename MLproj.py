...
class SecurityEvaluator:
    """Evaluate the effectiveness of Green-GAN for security testing"""
    ...
    def evaluate_detection_capability(self):
        """Test discriminator's ability to detect real vs synthetic attacks"""
        ...
        with torch.no_grad():
            # Test on real data (should output close to 1)
            real_scores = self.discriminator(real_tensor).cpu().numpy()
            # Test on synthetic data (ideally should be fooled, output close to 1)
            synthetic_scores = self.discriminator(synthetic_tensor).cpu().numpy()
        
        real_accuracy = np.mean(real_scores > 0.5)  # Should be high
        synthetic_fooling_rate = np.mean(synthetic_scores > 0.5)  # Generator success
    ...
    def visualize_results(self):
        ...
        # Feature comparison (first 10 features)
